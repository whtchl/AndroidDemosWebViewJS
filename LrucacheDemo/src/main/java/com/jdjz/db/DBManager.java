...
public class DBManager {
    ...
    public static DBManager init(Context context) {
        if (isInitialized) {
            JUtils.Log(TAG, "DBManager has init");
            return instance;
        }
        JUtils.Log(TAG, "DBManager init");
        mContext = context;
    ...
    private static String getDbPath () {
        String currentUserId = JUtils.getSharedPreference().getString(SealConst.SEALTALK_LOGIN_ID,null);//mContext.getSharedPreferences("config", MODE_PRIVATE).getString(SealConst.SEALTALK_LOGIN_ID, null);
        String dbPath = mContext.getFilesDir().getAbsolutePath();
        dbPath = dbPath + File.separator + getAppKey() + File.separator + currentUserId;
        JUtils.Log(TAG, "DBManager dbPath = " + dbPath);
        return  dbPath;
    }

    public void uninit() {
        JUtils.Log(TAG, "DBManager uninit");
