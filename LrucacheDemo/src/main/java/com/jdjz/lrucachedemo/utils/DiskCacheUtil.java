...
import com.jdjz.lrucachedemo.io.DiskLruCache;
...
/**
 * Created by happen on 2018-07-20.
 */
...
public class DiskCacheUtil {
    ...
    public void showImageByAsyncTask(ImageView iv, final String url) throws IOException {
        Log.i("jdjz","showImageByAsyncTask");
        //从缓存中取出图片
        Bitmap bitmap = getBitmapFromDiskCache(url);
        //如果缓存中没有，则需要从网络中下载
        if (bitmap == null) {
            Log.i("jdjz","showImageByAsyncTask  如果缓存中没有 设置为默认");
            iv.setImageResource(R.mipmap.ic_launcher);
        } else {
            //如果缓存中有 直接设置
            Log.i("jdjz","showImageByAsyncTask  如果缓存中有 直接设置");
    ...
    public void loadImages(int start, int end) throws IOException {
        Log.i("tchl","DiskCacheUtil loadImages");
        for (int i = start; i < end; i++) {
            ...
            //如果缓存中没有，则需要从网络中下载
            if (bitmap == null) {
                Log.i("tchl","DiskCacheUtil loadImages 如果缓存中没有，则需要从网络中下载");
                NewsAsyncTask task = new NewsAsyncTask(url);
                task.execute(url);
                mTaskSet.add(task);
            } else {
                //如果缓存中有 直接设置
                Log.i("tchl","DiskCacheUtil loadImages 如果缓存中有 直接设置");
    ...
    private Bitmap addBitmapToDiskCache(String url) throws IOException {
        ...
        if (Looper.myLooper() == Looper.getMainLooper()) {
            throw new RuntimeException("can not visit network from UI Thread."+" thread id:"+ Thread.currentThread().getId()+"  name:"+Thread.currentThread().getName());
        }
        if (mDiskCache == null) {
            return null;
        }
        Log.d("jdjz","addBitmapToDiskCache  thread id:"+ Thread.currentThread().getId()+"  name:"+Thread.currentThread().getName());
    ...
    private Bitmap getBitmapFromDiskCache(String url) throws IOException {
        ...
        Log.d("tchl","getBitmapFromDiskCache  thread id:"+ Thread.currentThread().getId()+"  name:"+Thread.currentThread().getName());
    ...
    private class NewsAsyncTask extends AsyncTask<String, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(String... params) {
            Log.i("tchl","DiskCacheUtil  doInBackground");
